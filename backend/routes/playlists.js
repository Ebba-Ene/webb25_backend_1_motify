...
router.get('/latest', async (req, res) => {
  try {
    const playlists = await Playlist.find({ owner: null })
      .sort({ _id: -1 })
      .limit(5)
...
router.get('/my/sharedPlaylists/:id', requireAuth, isPlaylistSharedWithUser, async (req, res) => {
  try{
    console.log("Hej")
    const playlists = await Playlist.find({ users: req.user._id })
      .populate('songs', 'title artist durationSeconds')
      .populate('owner', 'email').lean()
    res.json(playlists);
  }catch (err) {
    console.error('Shared playlists failed:', err.message);
    res.status(500).json({ error: err.message });
  }
})
...
//Owner can add users
router.patch('/my/:id', requireAuth, isPlaylistOwner, async (req,res) => {
    try {
      const playlist = await Playlist.findByIdAndUpdate(
        req.params.id,
